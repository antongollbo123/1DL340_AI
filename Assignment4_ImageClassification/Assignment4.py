...
def myGetModel(data):
    dimension = data.input_dim
    x_train, y_train = data.x_train, data.x_test
    ...
    opt = tf.keras.optimizers.SGD(learning_rate=0.002, momentum = 0.95, decay = 1e-6)
...
def myFitModel(model,data):
    ...
    model.fit(x_train,y_train, epochs = 50, batch_size=64, validation_data=(x_valid, y_valid), verbose=1, callbacks = my_callbacks)
